      ...
      (function(global2, factory) {
        ...
        function tempTransaction(db, mode, storeNames, fn) {
          if (!db.idbdb || !db._state.openComplete && (!PSD.letThrough && !db._vip)) {
            if (db._state.openComplete) {
              return rejection(new exceptions2.DatabaseClosed(db._state.dbOpenError));
            }
            if (!db._state.isBeingOpened) {
              if (!db._state.autoOpen)
                return rejection(new exceptions2.DatabaseClosed());
              db.open().catch(nop);
            }
        ...
        var Table = (function() {
          ...
          Table2.prototype.get = function(keyOrCrit, cb) {
            ...
            if (keyOrCrit == null)
              return rejection(new exceptions2.Type("Invalid argument to Table.get()"));
          ...
          Table2.prototype.update = function(keyOrObject, modifications) {
            if (typeof keyOrObject === "object" && !isArray(keyOrObject)) {
              ...
              if (key === void 0)
                return rejection(new exceptions2.InvalidArgument("Given object does not contain its primary key"));
          ...
          Table2.prototype.bulkAdd = function(objects, keysOrOptions, options) {
            ...
            return this._trans("readwrite", function(trans) {
              ...
              if (keyPath && keys2)
                throw new exceptions2.InvalidArgument("bulkAdd(): keys argument invalid on tables with inbound keys");
              if (keys2 && keys2.length !== objects.length)
                throw new exceptions2.InvalidArgument("Arguments objects and keys must have the same length");
          ...
          Table2.prototype.bulkPut = function(objects, keysOrOptions, options) {
            ...
            return this._trans("readwrite", function(trans) {
              var _a2 = _this.schema.primKey, auto = _a2.auto, keyPath = _a2.keyPath;
              if (keyPath && keys2)
                throw new exceptions2.InvalidArgument("bulkPut(): keys argument invalid on tables with inbound keys");
              if (keys2 && keys2.length !== objects.length)
                throw new exceptions2.InvalidArgument("Arguments objects and keys must have the same length");
              var numObjects = objects.length;
          ...
          function addConfiguredEvents(cfg) {
            keys(cfg).forEach(function(eventName) {
              ...
              if (isArray(args)) {
                ...
                throw new exceptions2.InvalidArgument("Invalid event config");
            });
          }
        }
        ...
        function getIndexOrStore(ctx, coreSchema) {
          ...
          if (!index)
            throw new exceptions2.Schema("KeyPath " + ctx.index + " on object store " + coreSchema.name + " is not indexed");
          return index;
        }
        ...
        function createWhereClauseConstructor(db) {
          return makeClassConstructor(WhereClause.prototype, function WhereClause2(table2, index, orCollection) {
            ...
            if (!this._IDBKeyRange)
              throw new exceptions2.MissingAPI();
          });
        }
        ...
        var Transaction = (function() {
          ...
          Transaction2.prototype.create = function(idbtrans) {
            ...
            if (!this.active)
              throw new exceptions2.TransactionInactive();
          ...
          Transaction2.prototype._promise = function(mode, fn, bWriteLock) {
            ...
            if (mode === "readwrite" && this.mode !== "readwrite")
              return rejection(new exceptions2.ReadOnly("Transaction is readonly"));
            if (!this.active)
              return rejection(new exceptions2.TransactionInactive());
            if (this._locked()) {
              return new DexiePromise(function(resolve, reject) {
          ...
          Version2.prototype._parseStoresSpec = function(stores, outSchema) {
            keys(stores).forEach(function(tableName) {
              if (stores[tableName] !== null) {
                ...
                primKey.unique = true;
                if (primKey.multi)
                  throw new exceptions2.Schema("Primary key cannot be multi-valued");
                indexes.forEach(function(idx) {
                  if (idx.auto)
                    throw new exceptions2.Schema("Only primary key can be marked as autoIncrement (++)");
                  if (!idx.keyPath)
                    throw new exceptions2.Schema("Index must have a name and cannot be an empty string");
                });
        ...
        function dexieOpen(db) {
          ...
          function throwIfCancelled() {
            if (state.openCanceller !== openCanceller)
              throw new exceptions2.DatabaseClosed("db.open() was cancelled");
          }
        ...
        function extractTransactionArgs(mode, _tableArgs_, scopeFunc) {
          ...
          if (i < 2)
            throw new exceptions2.InvalidArgument("Too few arguments");
        ...
        var Dexie$1 = (function() {
          ...
          Dexie3.prototype.version = function(versionNumber) {
            if (isNaN(versionNumber) || versionNumber < 0.1)
              throw new exceptions2.Type("Given version is not a positive number");
            versionNumber = Math.round(versionNumber * 10) / 10;
            if (this.idbdb || this._state.isBeingOpened)
              throw new exceptions2.Schema("Cannot add version when database is open");
          ...
          Dexie3.prototype.delete = function(closeOptions) {
            ...
            return new DexiePromise(function(resolve, reject) {
              ...
              if (hasInvalidArguments)
                throw new exceptions2.InvalidArgument("Invalid closeOptions argument to db.delete()");
              if (state.isBeingOpened) {
          ...
          Dexie3.prototype._transaction = function(mode, tables, scopeFunc) {
            ...
            try {
              ...
              if (mode == "r" || mode === READONLY)
                ...
                throw new exceptions2.InvalidArgument("Invalid transaction mode: " + mode);
              if (parentTransaction) {
                if (parentTransaction.mode === READONLY && idbMode === READWRITE) {
                  if (onlyIfCompatible) {
                    parentTransaction = null;
                  } else
                    throw new exceptions2.SubTransaction("Cannot enter a sub-transaction with READWRITE mode when parent transaction is READONLY");
                }
                if (parentTransaction) {
                  storeNames.forEach(function(storeName) {
                    if (parentTransaction && parentTransaction.storeNames.indexOf(storeName) === -1) {
                      if (onlyIfCompatible) {
                        ...
                        throw new exceptions2.SubTransaction("Table " + storeName + " not included in parent transaction.");
                    }
                  });
                }
