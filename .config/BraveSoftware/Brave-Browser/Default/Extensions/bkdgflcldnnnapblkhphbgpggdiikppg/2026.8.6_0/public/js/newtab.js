...
(() => {
  ...
  var dataFormatSchema = zod_default.object({
    atb: zod_default.string().optional(),
    totalCount: zod_default.number(),
    totalPeriod: zod_default.enum(["install-time"]),
    trackerCompaniesPeriod: zod_default.enum(["last-hour", "last-day"]),
    trackerCompanies: zod_default.array(
      zod_default.object({
        displayName: zod_default.string(),
        count: zod_default.number()
      })
    )
  });
  var disconnectMessage = zod_default.object({
    messageType: zod_default.literal(events.outgoing.newTabPage_disconnect)
  });
  var dataMessage = zod_default.object({
    messageType: zod_default.literal(events.outgoing.newTabPage_data),
    options: dataFormatSchema
  });
  var outgoing = zod_default.discriminatedUnion("messageType", [dataMessage, disconnectMessage]);
  var heartbeatMessage = zod_default.object({
    messageType: zod_default.literal(events.incoming.newTabPage_heartbeat)
  });
  var incoming = zod_default.discriminatedUnion("messageType", [heartbeatMessage]);
